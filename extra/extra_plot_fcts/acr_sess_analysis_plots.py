...
#############################################
def plot_data_signif(ax, sess_ns, sig_comps, maxes, lin_p_vals=None, 
    ...
    Optional args:
        - lin_p_vals       : p-values for each within line comparison, 
...
def plot_area_diff_acr_sess(analyspar, sesspar, stimpar, permpar, extrapar, 
    ...
    Required args:
        ...
        - extrapar (dict) : dictionary containing additional analysis 
                              parameters
            ["analysis"] (str): analysis type (e.g., "s" or "l")
    ...
    if plot == "grped":
        p_val_thr_corr = permpar["p_val"]
        if permpar["multcomp"]:
            p_val_thr_corr /= permpar["multcomp"]
    ...
    if plot == "grped":
        plot_data_signif(ax, sess_ns, sig_comps, maxes, diff_info["lin_p_vals"], 
            p_val_thr=permpar["p_val"], n_comps=permpar["multcomp"])
...
def plot_traces(sub_ax, xran, trace_st, lock=False, col="k", lab=True, 
    ...
    Required args:
        ...
        - trace_st (3D array)      : trace statistics, structured as
...
def plot_traces_acr_sess(analyspar, sesspar, stimpar, extrapar, sess_info, 
    ...
    Required args:
        ...
        - extrapar (dict) : dictionary containing additional analysis 
                            parameters
            ["analysis"] (str): analysis type (e.g., "t" or "r")
            ["datatype"] (str): datatype (e.g., "run", "roi")
        - sess_info (nested list): nested list of dictionaries for each 
                                   line/plane x mouse containing information 
                                   from each session, with None for missing 
...
#############################################
def plot_stim_idx_hist(sub_ax, percs, n_bins=None, data=None, rand_data=None, 
                       orig_edges=None, p_val=0.05, tails="2", plot="items", 
                       col="r", density=False, add_perc_label=True):
    """
    plot_stim_idx_hist(sub_ax, percs)
    ...
    Optional args:
        ...
        - orig_edges (None)     : origin edges used to generate counts 
                                  ...
                                  default: None
        - p_val (float)         : p-value for confidence interval
                                  default: 0.05
        - tails (str)           : tails for confidence interval
                                  default: "2"
        ...
        - density (bool)        : if True, densities are plotted instead of 
                                  frequencies.
                                  default: False
        - add_perc_label (bool) : if True, percentage significant items is 
                                  added to plots as a label
                                  default: True
    ...
    if plot != "skip" and add_perc_label:
...
def plot_stim_idx_acr_sess(analyspar, sesspar, stimpar, permpar, extrapar, 
    ...
    subtitle = (f"{dim_str} {feature_str}{common_str} {plot_str}")
...
def plot_unexp_latency(analyspar, sesspar, stimpar, permpar, latpar, extrapar, 
    ...
    plot_data_signif(
        ax, sess_ns, sig_comps, maxes, lat_data["lin_p_vals"], 
        p_val_thr=permpar["p_val"], n_comps=permpar["multcomp"])
