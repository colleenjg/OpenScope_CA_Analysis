...
ANALYSIS_DESCR = {
    ...
    "t": "running traces by session quantile, split by unexpected and expected sequences",
    "l": "running traces by session quantile, locked to unexpected or expected sequence onset",
...
def get_analysis_fcts():
    ...
    # 1. Analyses and plots average running by quantile x unexpected for each 
    # session 
    fct_dict["t"] = gen_analys.run_traces_by_qu_unexp_sess

    # 2. Analyses and plots average running locked to unexpected by quantile x 
